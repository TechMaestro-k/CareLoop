...
export default function DoctorInboxPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [r, e] = await Promise.all([api.listProposals({}), api.listEscalations("pending")]);
  ...
  const pendingPicks = items.filter(
    (i) => i.patient_status === "chosen" && i.doctor_status === "pending",
  );
  const accepted = items.filter((i) => i.doctor_status === "accepted");
  const awaitingPatient = items.filter((i) => i.patient_status === "pending");

  useEffect(() => {
    const missing = pendingPicks.filter((p) => !p.doctor_handoff_summary && !summaries[p.id]);
    ...
    (async () => {
      for (const p of missing) {
        try {
          ...
          if (cancelled) return;
          const summary =
            res?.doctor_handoff_summary || res?.proposal?.doctor_handoff_summary || null;
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink-40">
            Escalations from the agent, slot picks waiting on you, and confirmed bookings.
          </p>
        </div>
            ...
            <CardContent className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Button asChild size="sm">
                  <Link to={escalationDeeplink(e)}>Open</Link>
                </Button>
                <Button asChild size="sm" variant="outline">
                  <Link to={`/doctor/escalations/${e.id}`}>Review escalation</Link>
                </Button>
              </div>
            </CardContent>
          </Card>
      ...
      <Section
        title="Awaiting your accept"
        empty="No patient picks waiting on you."
          ...
          <Card key={p.id} className="triage-bar-amber">
            <CardHeader className="pb-2">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <CardTitle>
                  ...
                  <span className="text-sm font-normal text-ink-40">
                    · {p.patient?.age}y · {p.patient?.phone}
                  </span>
                </CardTitle>
                <div className="flex items-center gap-2">
            ...
            <CardContent className="space-y-3">
              <div className="text-sm">
                ...
                <span className="font-medium text-ink-DEFAULT">{p.chosen_slot?.human}</span>
              </div>
              {!isPaid && (
                <div className="rounded-lg border border-triage-amber/30 bg-triage-amberSoft px-3 py-2 text-xs text-triage-amber">
                  Patient has not paid the consult fee yet. You can simulate payment below for demo purposes.
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-2">
                {!isPaid && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => simulatePayment(p.id)}
                    disabled={working !== null}
                  >
                    {working === `${p.id}:pay` ? "Processing…" : "Simulate Payment (demo)"}
                  </Button>
                )}
                <Button
                  size="sm"
                  ...
                  title={!isPaid ? "Patient must pay first" : "Accept this booking"}
                >
                  <CheckCircle2 className="h-4 w-4" />
                  {working === `${p.id}:accept` ? "Confirming…" : "Accept"}
                </Button>
                <Button
                  size="sm"
                  variant="outline"
      ...
      >
        {accepted.map((p) => (
          <Card key={p.id} className="triage-bar-green">
            ...
            <CardContent className="space-y-2">
              <div className="text-sm font-medium text-ink-DEFAULT">{p.chosen_slot?.human}</div>
              <div className="flex flex-wrap gap-2">
                {p.jitsi_link && (
                  <a
                    href={p.jitsi_link}
...
function HandoffSummaryBlock({
  ...
  const wrapperClass = "rounded-lg border border-mint/40 bg-mint-soft/60 p-3 space-y-2 text-sm";
  const headerClass = "flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-mint-ink";
    ...
    return (
      <div className={wrapperClass}>
        ...
        <div className="text-xs text-triage-red">
          Could not load summary. Review patient record manually.
        </div>
      </div>
    );
  }
